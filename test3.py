...
#d= collection.find({'status': {'$in':['A','P']}}) # this is a filter using the inbuilt keyword of MongoDB which begins with '$'
#d= collection.find({'status': {'$gt':"C"}}) # '$gt -> gretater than
#d = collection.find({'qty':{'$gte':75}}) #here '$get' means greater then or equal to
...
#collection.delete_one({'item':'sudhanshu'}) ## to delete one item
